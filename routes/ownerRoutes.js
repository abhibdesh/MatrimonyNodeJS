    ...
    try {
      if (req.user.__t === "owner") {
        ...
        return res
          ...
          .json({
            message: "failure",
            data: "You are unauthorised to get payments",
          });
      }
    } catch (error) {
      console.log(error)
    ...
    try {
      if (req.user.__t === "owner") {
        ...
        return res
          ...
          .json({
            message: "success",
            data: "This payment has been approved successfully.",
          });
      } else {
        return res
          .status(401)
    ...
    try {
      if (req.user.__t === "owner") {
        ...
        await Payment.updateMany(
          ...
          { $set: { isPaymentSettled: true } }
        );
        return res
          .status(200)
          .json({
            message: "success",
            data: "Payment Settlement Marked successfully",
          });
      } else {
        return res
          .status(200)
          .json({
            message: "failure",
            data: "You are unauthorised to settle payments",
          });
      }
    } catch (error) {
      console.log(error)
    ...
    try {
      if (req.user.__t === "owner") {
        ...
        return res
          ...
          .json({
            message: "failure",
            data: "You are unauthorised to add new menu",
          });
      }
    } catch (error) {
      console.log(error)
